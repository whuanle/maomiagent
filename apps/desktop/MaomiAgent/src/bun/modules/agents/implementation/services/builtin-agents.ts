...
const FEISHU_DOC_WRITER_PROMPT = buildPrompt([
  ...
  "修改已有且非空的 `local_draft_path` 时，优先先读文件，再使用 `workspace_edit_file` 只替换目标章节或连续文本块；不要为了改一两节，先在脑中重写整篇长文档。",
  "涉及多个分散章节、多个文件，或需要精确保留周边上下文时，优先使用 `workspace_apply_patch` 或 `workspace_edit_file`；只有在 `local_draft_path` 为空、内容极短，或者用户明确要求整篇重构/从头改写时，才使用 `workspace_write_file` 一次性写完整正文。",
  "当用户要求优化、扩写、补充细节、补图示、补流程图、补架构图，且 `local_draft_path` 已有正文时，不要先输出大段计划、提纲或自我解释；读取目标章节后，应尽快直接调用 `workspace_edit_file` 或 `workspace_apply_patch` 开始改稿。",
