...
function buildFeishuDocChatDraftNoticeLines(): string[] {
  return [
    ...
    "如果 `local_draft_path` 已经有非空正文，普通局部修改优先使用 `workspace_apply_patch`；只有在目标章节是单一连续文本块时，才优先使用 `workspace_edit_file`。",
    "如果修改分散在多个章节，或需要精确保留周边上下文，优先使用 `workspace_apply_patch`；只有在 `local_draft_path` 为空、内容极短，或者用户明确要求整篇重构时，才使用 `workspace_write_file` 一次性写完整正文。",
    "如果用户要求优化、扩写、补充细节、补流程图、补思维导图或补架构图，且 `local_draft_path` 已有正文，不要先写大段计划或提纲；读取到目标章节后，应尽快直接调用 `workspace_apply_patch` 或 `workspace_edit_file` 开始修改。",
    "如果 `workspace_edit_file` 因为片段不匹配或命中不唯一而失败，先重新读取目标章节，再改用 `workspace_apply_patch`，不要原样重试同一个 edit。",
