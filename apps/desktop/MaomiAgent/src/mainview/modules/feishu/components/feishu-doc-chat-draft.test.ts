...
describe("buildFeishuDocChatDraftText", () => {
  test("reserves leading space for the user task and keeps the caution block above the document context", () => {
    ...
    expect(draft).toContain("如果 `local_draft_path` 已经有非空正文，普通局部修改优先使用 `workspace_apply_patch`")
    expect(draft).toContain("只有在目标章节是单一连续文本块时，才优先使用 `workspace_edit_file`")
    ...
    expect(draft).toContain("如果 `workspace_edit_file` 因为片段不匹配或命中不唯一而失败")
