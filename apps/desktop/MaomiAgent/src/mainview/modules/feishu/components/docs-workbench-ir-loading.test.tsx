...
describe("Feishu docs workbench IR loading bridge", () => {
  ...
  test("workbench opens and pulls workspace markdown for visual preview", async () => {
    ...
    expect(workbench).toContain("当前依据：飞书原始结构 + 结构化基线");
    expect(workbench).toContain("推荐发布方式：先重新拉取远端基线");
    expect(workbench).toContain("const workspaceDiagnostic = useMemo(() => buildWorkspaceDiagnostic(currentDoc), [currentDoc])");
