...
export function buildTaskTableColumns(input: Input): TableColumnsType<DesktopTaskCenterItem> {
  ...
  return [
      ...
      render: (_, row) => (
        <div className="tasks-page-task-cell">
          ...
          <Space size={[8, 8]} wrap className="tasks-page-task-meta">
            ...
            {row.linkedSessionId ? <Text type="secondary">{row.linkedSessionId}</Text> : null}
          </Space>
        </div>
      ),
    },
    {
      ...
      render: (_, row) => (
        <div className="tasks-page-mode-cell">
          ...
          <Text type="secondary">
            {`${priorityLabel(t, row.priority)} / ${taskCenterPhaseLabel(t, row)}`}
          </Text>
        </div>
      ),
    },
    ...
    {
      title: t("任务页.字段.handler"),
      key: "handler",
      width: 220,
      render: (_, row) => (
        <div className="tasks-page-executor-cell">
          <Text>{taskCenterHandlerLabel(row)}</Text>
          <Text type="secondary">{taskCenterModuleLabel(row)}</Text>
        </div>
      ),
    },
    {
      ...
      width: 168,
      render: (_, row) => (
        <Text type="secondary">{formatDateTime(row.updatedAt)}</Text>
      ),
    },
    {
      title: t("任务页.列.进度"),
    ...
    {
      title: t("任务页.字段.blockedBy"),
      key: "blockedReason",
      width: 220,
      render: (_, row) => (
        <Text type="secondary">{taskCenterAttentionReason(row) || "-"}</Text>
      ),
    },
    {
      title: t("任务页.列.操作"),
